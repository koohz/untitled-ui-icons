const React = require("react");
function Disc02Icon(props) {
  return /*#__PURE__*/React.createElement("svg", Object.assign({
    width: 24,
    height: 24,
    fill: "none",
    viewBox: "0 0 24 24"
  }, props), /*#__PURE__*/React.createElement("path", {
    fill: "currentColor",
    fillOpacity: 0.01,
    d: "M12 20C16.9706 20 21 15.9706 21 11C21 6.02944 16.9706 2 12 2C7.02944 2 3 6.02944 3 11C3 15.9706 7.02944 20 12 20Z"
  }), /*#__PURE__*/React.createElement("path", {
    fill: "currentColor",
    fillOpacity: 0.01,
    d: "M12 13.7C13.4912 13.7 14.7 12.4912 14.7 11C14.7 9.50883 13.4912 8.3 12 8.3C10.5088 8.3 9.3 9.50883 9.3 11C9.3 12.4912 10.5088 13.7 12 13.7Z"
  }), /*#__PURE__*/React.createElement("path", {
    stroke: "currentColor",
    strokeLinecap: "round",
    strokeLinejoin: "round",
    strokeWidth: 2,
    d: "M14.7 5.30627C16.8285 6.31739 18.3 8.48684 18.3 11M8.75317 16.4C6.92362 15.2975 5.7 13.2916 5.7 11M21 11C21 15.9706 16.9706 20 12 20C7.02944 20 3 15.9706 3 11C3 6.02944 7.02944 2 12 2C16.9706 2 21 6.02944 21 11ZM14.7 11C14.7 12.4912 13.4912 13.7 12 13.7C10.5088 13.7 9.3 12.4912 9.3 11C9.3 9.50883 10.5088 8.3 12 8.3C13.4912 8.3 14.7 9.50883 14.7 11Z"
  }));
}
module.exports = Disc02Icon;